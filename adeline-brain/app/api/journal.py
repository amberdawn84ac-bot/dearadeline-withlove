...
async def seal_journal(body: SealRequest, student_id: str = Depends(get_current_user_id)):
    try:
        track_progress = await journal_store.seal(student_id=student_id, lesson_id=body.lesson_id, track=body.track.value, completed_blocks=body.completed_blocks, sources=body.evidence_sources or None)
    ...
    return SealResponse(sealed=True, lesson_id=body.lesson_id, track=body.track, track_progress=track_progress)
...
async def list_daily_notes(student_id: str, limit: int = Query(50, ge=1, le=200), _user_id: str = Depends(verify_student_access)):
    try:
        notes = await journal_store.get_daily_notes(student_id, limit=limit)
        return DailyNotesResponse(student_id=student_id, notes=[DailyNoteItem(**note) for note in notes])
    except Exception as exc:
...
async def _update_card_safe(student_id: str, body: SealRequest) -> None:
    try:
        ...
        await update_card_after_lesson(student_id=student_id, concept_id=concept_id, concept_name=concept_name, track=body.track.value, quality=quality)
    except Exception as exc:
...
async def get_recent(student_id: str, limit: int = 10, _user_id: str = Depends(verify_student_access)):
    try:
        entries = await journal_store.get_recent(student_id, limit=min(limit, 50))
    except Exception as exc:
        logger.exception("[/journal/recent] DB error")
        raise HTTPException(status_code=500, detail=str(exc))
    return RecentResponse(student_id=student_id, entries=[RecentEntry(**entry) for entry in entries])
