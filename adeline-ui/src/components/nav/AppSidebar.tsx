...
import { BookOpen, Settings, Menu, X, Trophy, Hammer, Archive } from "lucide-react";
    ...
    <div className="min-h-screen bg-[#FFFEF7] flex flex-col md:flex-row">
      <div className="md:hidden flex items-center justify-between p-4 border-b border-[#E7DAC3] bg-white/50 backdrop-blur-sm sticky top-0 z-50">
        <Link href="/dashboard" className="flex items-center gap-3">
          <span className="font-bold text-[#2F4731]" style={{ fontFamily: "var(--font-emilys-candy), cursive" }}>Dear Adeline</span>
        </Link>
      ...
      <aside className={[
        ...
        <div className="flex flex-col h-full p-6">
          <div className="hidden md:flex items-center gap-3 mb-10 px-2">
            <div className="w-12 h-12 rounded-xl bg-[#2F4731] flex items-center justify-center shadow-md border-2 border-white"><span className="text-2xl">🌿</span></div>
            <div className="flex flex-col">
              <span className="font-bold text-xl text-[#2F4731] leading-none" style={{ fontFamily: "var(--font-emilys-candy), cursive" }}>Dear Adeline</span>
              <span className="text-[10px] font-black uppercase tracking-widest text-[#BD6809] mt-1">Learning Hub</span>
            </div>
          </div>
          ...
          <nav className="space-y-1">
            {NAV_ITEMS.map((item) => {
              ...
              return (
                <Link key={item.href} href={item.href} onClick={() => setIsOpen(false)} className={[
                  "flex items-center gap-3 px-4 py-3 rounded-xl transition-all duration-200 group",
                  isActive ? "bg-[#2F4731] text-white shadow-lg font-bold" : "text-[#2F4731]/70 hover:bg-[#2F4731]/5 hover:text-[#2F4731] font-medium",
                ].join(" ")}>
                  <Icon size={20} className={["transition-transform group-hover:scale-110", isActive ? "text-[#BD6809]" : ""].join(" ")} />
          ...
          <div className="mt-6 mb-6"><DailyBreadWidget onStudy={handleDailyBreadStudy} /></div>

          <div className="mt-auto pt-6 border-t border-[#E7DAC3]">
            <Link href="/dashboard/settings" className="flex items-center gap-3 px-4 py-2 text-sm font-medium text-[#2F4731]/60 hover:text-[#2F4731] transition-colors">
