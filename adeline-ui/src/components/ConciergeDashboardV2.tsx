...
export default function ConciergeDashboardV2({ studentId, studentName, gradeLevel }: Props) {
  ...
  const [messages, setMessages] = useState<ChatMessage[]>([
    { id: 'welcome', role: 'adeline', text: `Hey ${firstName}. What have you been up to? You can tell me anything.` },
  ]);
  ...
  useEffect(() => {
    listActivities(studentId).then((result) => {
      setActivities(result.activities.slice(0, 4));
      setActivityCredits(result.total_credits);
    }).catch(() => undefined);
  }, [studentId]);
  ...
  async function fileActivity(description: string, minutes: number) {
    ...
    setMessages((prev) => [...prev, { id: makeId(), role: 'adeline', text: result.adeline_note, credit: result }]);
    setActivities((prev) => [{
  ...
  async function send() {
    ...
    try {
      if (pendingActivity) {
        ...
        if (!minutes) {
          setMessages((prev) => [...prev, { id: makeId(), role: 'adeline', text: 'About how long did it take? A rough guess is fine.' }]);
          return;
        }
        const description = pendingActivity;
        setPendingActivity(null);
        await fileActivity(description, minutes);
        return;
      }
      ...
      if (ACTIVITY_RE.test(text)) {
        ...
        if (!minutes) {
          ...
          setMessages((prev) => [...prev, { id: makeId(), role: 'adeline', text: 'That counts for more than you might think. About how long were you working on it?' }]);
          return;
        }
      ...
      for await (const event of streamConversation({ studentId, message: text, gradeLevel, history })) {
        if (event.type === 'text') {
          ...
          setMessages((prev) => prev.map((message) => message.id === streamingId ? { ...message, text: responseText } : message));
        } else if (event.type === 'block' && event.block_type === 'SKETCHNOTE_NOTE') {
          const note = normalizeSketchnote(event as Record<string, unknown>);
          setMessages((prev) => prev.map((message) => message.id === streamingId ? { ...message, sketchNote: note } : message));
      ...
      setHistory((prev) => [...prev, ...turns].slice(-12));
    } catch {
      setMessages((prev) => [...prev, { id: makeId(), role: 'adeline', text: 'Something tripped over itself. Try that once more.' }]);
    } finally {
      setBusy(false);
    }
  }
          ...
          <div className="border-t border-[#d9cfbb] bg-[#f7f1e4] px-4 py-4 sm:px-8">
            <div className="mx-auto max-w-4xl">
              <div className="flex items-end gap-3 rounded-[24px_18px_24px_18px] border border-[#cfc2ab] bg-[#fffdf7] p-3 shadow-[0_8px_24px_rgba(67,51,31,.07)]">
                ...
                <textarea value={input} onChange={(event) => setInput(event.target.value)} onKeyDown={(event) => { if (event.key === 'Enter' && !event.shiftKey) { event.preventDefault(); void send(); } }} rows={2} placeholder={pendingActivity ? 'About how long did it take?' : 'Tell Adeline what happened, what you made, what you are wondering about...'} className="min-h-[52px] flex-1 resize-none bg-transparent px-1 py-2 text-sm outline-none placeholder:text-[#9a8b76]" />
