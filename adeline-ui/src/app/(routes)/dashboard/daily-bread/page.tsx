...
export default function DailyBreadLessonPage() {
  ...
  return bread && <div className="mx-auto max-w-4xl space-y-6 pb-16">
    ...
    <header className="rounded-[28px] border border-[#E7DAC3] bg-[#FFFDF5] p-7 md:p-10">
      ...
      <blockquote className="mt-6 border-l-4 border-[#BD6809] pl-5 text-lg italic leading-8 text-[#2F4731]">“{study?.fox_text || bread.verse}”</blockquote>
      <p className="mt-2 font-bold text-[#BD6809]">{bread.reference}</p>
      {(study?.sefaria_url || bread.sourceUrl) && <a href={study?.sefaria_url || bread.sourceUrl} target="_blank" rel="noreferrer" className="mt-3 inline-flex text-xs font-semibold text-[#2F4731]/60 underline">{study?.is_fox || bread.isFoxTranslation ? 'Everett Fox text via Sefaria' : (bread.sourceVersion || 'Source text via Sefaria')} ↗</a>}
    </header>

    {(study?.hebrew_text || bread.originalText) && <section className="rounded-2xl border border-[#E7DAC3] bg-white p-6"><p className="text-xs font-black uppercase tracking-wide text-[#BD6809]">Original text</p><p dir="rtl" className="mt-3 text-2xl leading-10 text-[#2F4731]">{study?.hebrew_text || bread.originalText}</p></section>}
    ...
    <section className="rounded-2xl bg-[#EEF4E9] p-6"><h2 className="text-xl font-bold text-[#2F4731]">Live it today</h2><p className="mt-2 text-sm leading-6 text-[#2F4731]/75">{bread.practice}</p><p className="mt-5 italic text-[#2F4731]/70">{bread.prayer}</p></section>
