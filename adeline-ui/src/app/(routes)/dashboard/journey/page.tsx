...
export default function LearningPlanPage() {
  ...
  return <div className="space-y-7 pb-12">
    ...
    {plan && <>
      ...
      {view === 'month' && firstMonth && <section className="space-y-4"><div><h2 className="text-2xl text-[#2F4731]" style={{ fontFamily: 'var(--font-emilys-candy), cursive' }}>{firstMonth.label}: {firstMonth.focus}</h2><p className="mt-1 text-sm text-[#2F4731]/60">Every investigation includes reading, communication, mission-based math, hands-on discovery, and a portfolio finish.</p></div>{firstMonth.weeks.map((week) => <article key={week.week} className="rounded-2xl border border-[#E7DAC3] bg-white p-5"><p className="text-[11px] font-black uppercase tracking-[.14em] text-[#BD6809]">Weekly family investigation</p><h3 className="mt-1 font-bold text-[#2F4731]">Week {week.week} · {week.theme}</h3><div className="mt-3 grid gap-3 md:grid-cols-4">{week.days.map((day) => <Link key={day.date} href={`/dashboard/lesson/${encodeURIComponent(day.lesson_id)}`} className="rounded-xl bg-[#FFF8EC] p-3 hover:ring-2 hover:ring-[#BD6809]/30"><small className="font-black uppercase text-[#BD6809]">{day.day}</small><p className="mt-1 text-sm font-bold text-[#2F4731]">{day.emoji} {day.activity_kind}</p><span className="mt-2 block text-[11px] leading-4 text-[#2F4731]/55">{day.daily_rhythm.join(' · ')}</span></Link>)}</div></article>)}</section>}
