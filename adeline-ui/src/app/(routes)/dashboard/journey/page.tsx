...
export default function LearningPlanPage() {
  ...
  return <div className="space-y-7 pb-12">
    <header className="rounded-[28px] border border-[#D9CFBC] bg-white/80 p-7">
      ...
      <p className="mt-2 max-w-3xl text-sm leading-6 text-[#2F4731]/65">A 36-week, four-day path. Adeline builds each scheduled topic as a canonical family lesson when it is opened, then the registrar records demonstrated evidence toward credits.</p>
    ...
    {plan && <>
      <section className="grid gap-4 sm:grid-cols-3">
        ...
        <Stat label="This year" value={`${plan.roadmap.total_weeks} weeks`} note={`${plan.roadmap.school_days_per_week}-day family schedule`} />
      ...
      {view === 'year' && <section className="grid gap-4 md:grid-cols-3">{plan.roadmap.months.map((month) => <article key={month.month} className="rounded-2xl border border-[#E7DAC3] bg-white p-5"><small className="font-black uppercase text-[#BD6809]">{month.label}</small><h3 className="mt-2 font-bold text-[#2F4731]">{month.focus}</h3><p className="mt-2 text-sm text-[#2F4731]/60">Weeks {month.weeks[0].week}–{month.weeks.at(-1)?.week} · 16 family lesson days</p><ul className="mt-3 space-y-1 text-xs text-[#2F4731]/65">{month.weeks.map((week) => <li key={week.week}>Week {week.week}: {week.theme}</li>)}</ul></article>)}</section>}
